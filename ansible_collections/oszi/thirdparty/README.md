...
* All third-party sources must be disabled by default via `{role}_enabled` variables.
...
* Main task orchestration pattern:
  1. `{{ ansible_facts.os_family | lower }}/{{ {role}_enabled | ternary('install', 'uninstall') }}.yml`
